...
// ═══════════════════════════════════════════════════════════════════════════════
// GET /api/v1/servicios/comodatos
// ═══════════════════════════════════════════════════════════════════════════════

describe("GET /api/v1/servicios/comodatos — comodatos activos", () => {
  const comodatoRow = {
    ID_SERVICIO: 1, CURP: CURP_VALIDA,
    NOMBRE_BENEFICIARIO: "Juan Pérez", TIPO_SERVICIO: "Préstamo de equipo",
    NOMBRE_ARTICULO: "Silla de ruedas", ID_ARTICULO: 5, CANTIDAD: 1,
    FECHA: new Date("2026-05-01"), FECHA_DEVOLUCION_ESPERADA: new Date("2026-06-01"),
  };

  test("retorna lista con shape correcto (200)", async () => {
    ...
    expect(res.body[0]).toMatchObject({
      ...
      nombreArticulo: "Silla de ruedas",
      cantidad: 1,
    });
  });

  test("NOMBRE_ARTICULO nulo → nombreArticulo null", async () => {
    mockExecute.mockResolvedValueOnce({ rows: [{ ...comodatoRow, NOMBRE_ARTICULO: null }] });
    ...
    expect(res.status).toBe(200);
    expect(res.body[0].nombreArticulo).toBeNull();
  });
...
// ═══════════════════════════════════════════════════════════════════════════════
// PATCH /api/v1/servicios/:idServicio/devolucion
// ═══════════════════════════════════════════════════════════════════════════════
...
describe("PATCH /api/v1/servicios/:idServicio/devolucion — confirmar devolución", () => {
  ...
  test("idServicio inválido → 400", async () => {
    const res = await request(app)
      .patch("/api/v1/servicios/abc/devolucion")
      .set("Authorization", `Bearer ${tokenAdmin}`);

    expect(res.status).toBe(400);
  });

  test("servicio no encontrado → 404", async () => {
    mockExecute.mockResolvedValueOnce({ rows: [] }); // getById retorna vacío

    const res = await request(app)
      .patch("/api/v1/servicios/999/devolucion")
      .set("Authorization", `Bearer ${tokenAdmin}`);
