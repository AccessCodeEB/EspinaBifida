...
describe("findArticulosSinStock", () => {
  it("retorna artículos con stock cero", async () => {
  ...
  it("retorna [] cuando no hay artículos sin stock", async () => {
    mockExecute.mockResolvedValueOnce({ rows: [] });
    expect(await findArticulosSinStock()).toEqual([]);
  });
...
describe("syncSinStockConsolidado", () => {
  it("inserta notificación cuando no existe previa y hay mensaje", async () => {
  ...
  it("actualiza la notificación existente con nuevo mensaje", async () => {
