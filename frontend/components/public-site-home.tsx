...
// @ts-ignore - Ignora la falta de tipos ya que GradualBlur es un componente JS
...
export function PublicSiteHome() {
  ...
  <div className="relative flex justify-center">
    ...
    <div className="absolute -left-5 bottom-10 h-56 w-56 rounded-full border-[12px] border-[#f6b21a] opacity-80" />

    {/* Puntos decorativos */}
    <div className="absolute right-2 top-0 hidden grid-cols-4 gap-2 md:grid">
      {Array.from({ length: 16 }).map((_, index) => (
        <span
          key={index}
          className="h-1.5 w-1.5 rounded-full bg-[#f6b21a]"
        />
      ))}
    </div>

    {/* Imagen principal */}
    <div className="relative z-10 h-[360px] w-full overflow-hidden rounded-[2.5rem] rounded-bl-[6rem] shadow-2xl ring-1 ring-slate-200 md:h-[430px]">
      <img
        src="/spina-bifida-hero.jpg"
        alt="Atención integral en la Asociación de Espina Bífida"
        className="h-full w-full object-cover object-center"
      />

      {/* Overlay suave azul, no gris */}
      <div className="absolute inset-0 bg-gradient-to-t from-[#0f4c81]/35 via-transparent to-transparent" />
    </div>

    {/* Ícono flotante */}
    <div className="absolute -left-3 top-12 z-20 flex h-16 w-16 items-center justify-center rounded-full bg-[#0f4c81] text-3xl text-white shadow-xl">
      ♡
    </div>
  </div>
</RevealOnMount>
